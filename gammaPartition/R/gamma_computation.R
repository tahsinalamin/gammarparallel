...
## Description: Gamma matrix computation in blocks for input data set 
## Output: gamma matrix in "partition_gamma_1.txt" file
...
gamma.partition <- function(filename, sparse="Y") {
  ...
  print("**gamma computation started**")
  #print(sparse)

  #read the dataset
  partial_gamma <- matrix() #gamma for each chunk
  ChunkSize = 40 #set block size or chunk size
